...
- **Rodada 2, calibração (2026-09-20, o Igor decidiu não parar):** pré-registro em `docs/forex/fast2-preregistration.md` (commitado antes de qualquer resultado, com registro append-only e guarda de hash em `fast2-registry.jsonl`). Todas as famílias são calibradas numa **grade de 212 combinações** (`docs/forex/fast2-grid.json`, gerada por `fx_fast2_grid.py`; as 8 configurações da rodada 1 estão dentro, com parâmetros idênticos, e um teste confere), com a estatística da rodada 1 (bootstrap de dias com máximo-t e placebo com 200 conjuntos) pagando pelo número de combinações. **Três amostras:** descoberta em **2011-01 a 2018-11** (ticks do HistData de outra fonte, relógio no horário de verão dos EUA, conferido contra o oráculo da Dukascopy em 2017-2018 e pela hora da abertura semanal de 2011 a 2018; spread cerca de 0,1 pip maior, o que deixa os custos mais conservadores), réplica em 2019-01 a 2024-08 e confirmação única na amostra congelada 2024-09 a 2026-08. Código: `fx_fast2_lab.py` (`prepare`, `calibrate`, `discover`, `replicate`, `confirm`), `fx_fast2_manifest.py` (qualidade do dado de S0, com checagem de relógio sem oráculo), `fx_fast2_registry.py`. Duas otimizações sem mudar resultado (conferidas contra o relatório da rodada 1): a F3 só calcula as medianas nas barras que passam no teste de corpo (3× mais rápida) e `run_sample` reaproveita barras agregadas entre configurações. Uma réplica placebo das 212 combinações leva ~3 min por núcleo. **S0 fechada antes de qualquer estratégia:** 10 pares e 950 par-meses processados; o manifesto inclui 798 e exclui 152 pelos critérios fixos. Um teste escrito antes da correção encontrou uma falha de fronteira: o primeiro trecho de domingo de um mês UTC era confundido com a abertura semanal; a correção recuperou 98 par-meses sem afrouxar regra. Restaram 9 falhas reais de relógio, 19 buracos, 123 spreads acima de 3 pips e 13 falhas de oráculo; nenhuma cotação cruzada. Próximos passos: `prepare`, calibração (~1,5 a 2 h), descoberta, réplica e confirmação, com registro e relatório commitados em cada etapa. Palpite registrado antes do dado: 2% a 5% de chance de algo passar nas três amostras.
