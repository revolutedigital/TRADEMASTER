...
- **Step 28 concluído também em PostgreSQL 16 (2026-09-20):** migração aditiva `019_fx_bots` com 4 tabelas (bots, trades, snapshots de equity, eventos), sem coluna de credencial. Além dos testes em SQLite, foi validada num PostgreSQL descartável: cadeia completa até `018`, upgrade para `019`, inserção de bot e evento, rejeição da restrição de risco inválido, rollback para `018` removendo somente as quatro tabelas FX e preservando `orders`, `trades` e `strategy_deployments`, e novo upgrade para `019`. O container de teste foi removido ao final.
- **Auditoria independente das famílias (2026-09-20), corrigida antes de qualquer resultado real:** um auditor com contexto limpo comparou o código com o pré-registro regra a regra e rodou 130 mutações (99 mortas, 31 sobreviveram). Defeitos achados e corrigidos: (1) stop e alvo estavam ancorados no preço de execução e não nos níveis que a regra define (extremo da faixa, extremo do pico, média da janela); o simulador agora ancora no fechamento do mid da barra do sinal, recusa ordem cujo stop ou alvo já ficou para trás (como uma corretora) e registra a distância real do fill ao stop, e o executor do robô ancora igual; (2) uma entrada podia executar numa barra horas depois do sinal (fim de semana, buraco): agora é cancelada; (3) a regra de barra ausente do documento foi reescrita como o código a executa. Cobertura: novos testes para gap entre barras, spread variável, limites exatos de spread, corpo e número de barras da faixa, saídas da F1b e da F2b e as 8 colunas do cruzado sintético. Os padrões do simulador validado contra o G0 não mudaram (só ligam com `anchor_signal`). Também: o numba não guarda mais em cache as especializações sobre funções de estratégia (podiam falhar ao recarregar). Suíte unitária: 984 testes.
- **Comissão da Fusion corrigida (2026-09-20):** a Fusion cobra a comissão da conta Zero na cTrader **na moeda base do par** (2,25 por 100 mil; EURUSD 1 lote = EUR 2,25), e não US$ 2,25 fixo. O simulador e o laboratório passaram a usar isso (EURUSD ~+17%, GBPUSD ~+35%, AUDUSD ~-34% no custo de comissão). É um erro de fato no custo, achado antes de existir resultado, corrigido por emenda registrada; isso excede a letra de "nunca muda custo" do pré-registro, e por isso fica sinalizado ao Igor.
- **Step 4, pesquisa sem credenciais concluída (2026-09-20), em `docs/forex/ctrader-paths.md`:** o "cTrader Console" hoje é o **cTrader CLI** (lançado em 13/08/2026): roda cBots e tem comandos de ordem, posição e preço sem exigir aplicativo da Open API, mas cada comando de trading é um processo e um login (senha do cTID na linha de comando), sem stream de cotação. **Caminho de produção: cliente próprio em asyncio sobre o `.proto` da Open API** (o SDK Python oficial está parado desde 2024-06 e fixa versões antigas); o CLI entra como sonda do dia 0 na demo e como ferramenta independente de "fechar tudo". Ordem a mercado pela Open API não aceita SL/TP absoluto: usar SL/TP relativo ou emendar logo depois do fill (o executor já emenda e fecha se a proteção não vier). Aprovação do aplicativo é manual e sem prazo publicado: **registrar hoje**; se passar de 10 dias úteis, o canário roda pelo CLI em polling. A Fusion não define scalping nas cláusulas lidas, mas os contratos permitem anular trades e encerrar contas por "abusive trading" sem defini-lo; entidade que atende brasileiros: não achei. Lista do que o Igor faz e as 9 perguntas em inglês para a Fusion estão nas seções 5 e 6 do documento.
- **Step 5 concluído, F4 sai do escopo (2026-09-20), decisão conforme o pré-registro:** existe fonte oficial, gratuita e com termos compatíveis para NFP, CPI dos EUA, Fed, BCE e BoE (99,5% de cobertura de 2019-01 a 2026-08, horários conferidos contra o pico do M1 de EURUSD e GBPUSD, inclusive dos dois lados de cada troca de horário de verão), mas o BoJ não publica o horário da decisão em lugar nenhum (0% da categoria, 85,3% no agregado, contra os 90% exigidos). Com isso a família de notícia (F4, step 16) não entra e K continua 10. Achado útil: o BCE anunciava às 13:45 CET até 2022-07-20 e só passou a 14:15 em 2022-07-21; um 14:15 fixo erraria 28 das 61 decisões. Se o Igor quiser uma F4 mais estreita (NFP, CPI, Fed, BCE e BoE, com lista fixa), é uma hipótese nova que exige emenda registrada antes de qualquer resultado; a fonte técnica está pronta (`docs/forex/calendar-source.md`, `fx_calendar_probe.py`, 47 testes).
- **Step 4, adaptador da Open API escrito e testado só contra servidor falso (2026-09-20):** `app/fx/runner/ctrader/` (cliente asyncio com framing, correlação por `clientMsgId`, heartbeat, reconexão com reassinatura de cotações, limite de taxa e credenciais que não vazam em `repr` nem em erro; `CTraderVenue` que implementa a interface do runner) e `scripts/research/ctrader_probe.py`, que roda a matriz de testes da demo, recusa qualquer host que não seja a demo e nunca imprime credenciais. 84 testes, incluindo o executor e a reconciliação reais contra o servidor falso. **Não falou com a cTrader de verdade:** as suposições não verificadas estão no docstring de `client.py`, cada uma ligada a um item da matriz, e só o probe na demo as confirma. O executor e a reconciliação agora tratam a recusa do broker ao emendar o stop (fecha a posição e aciona o kill). `protobuf` está só no extra `research`; para o runner subir no Railway ele precisa ir para as dependências principais. O app da Open API do Igor está em "Submitted" (sem prazo); o caminho pelo cTrader CLI depende de o Docker responder nesta sessão (o grupo `docker` já inclui o usuário, mas a sessão do Claude Code começou antes).
- **Step 4, teste na demo Fusion pelo cTrader CLI feito (2026-09-20):** login sem bloqueio de 2FA, ordem a mercado de 1.000 unidades de EURUSD com SL e TP, e **o stop ficou gravado no servidor com o cliente desconectado** (conferido num login novo). Custo real do ciclo de 0,01 lote: 0,6 pip de spread e comissão de US$ 0,06 (coerente com "2,25 na moeda base"). A posição foi fechada e a conta ficou zerada. Resultado completo na seção 9 de `docs/forex/ctrader-paths.md`. Isto é o CLI (que usa a Open API por baixo); o adaptador próprio só pode ser provado com o app aprovado.
- **Steps 19 e 20, resultado (2026-09-20): nenhuma das 10 estratégias pré-registradas foi aprovada na descoberta; a amostra congelada não foi tocada.** Calibração antes: 0% de falsa aprovação em 300 conjuntos sem vantagem (limite 8%). Descoberta em 2019-01 a 2024-08, com o custo real da Fusion: todas têm R médio **negativo** depois do custo, no custo base e no de estresse: F1a -0,045 R por trade (6.358 trades, t = -2,95, 2 de 10 pares positivos), F1b -0,079, F2a -0,106, F2b -0,113, F3a -0,225, F3b -0,159, F5a -0,282 (317 trades, inconclusiva por poucos trades), F5b -0,442 (263 trades, inconclusiva) e os controles C1 -0,479 e C2 -0,636 (perderam como esperado, o que confirma que o teste não aprova qualquer coisa). Os efeitos mínimos detectáveis das seis famílias com muitos trades ficam entre 0,02 e 0,06 R: uma vantagem de 0,06 R por trade ou mais seria detectada com 80% de poder (significância unilateral de 0,5%). Relatório: `docs/forex/fast-discovery-report.md`. **Pela regra do pré-registro, paro e converso com o Igor:** não abro nova busca por conta própria.
- **Robôs de demo (2026-09-20):** sessão persistente do cTrader CLI como venue (`app/fx/runner/ctrader_cli.py`, ~0,5 s por ordem contra 20 a 30 s por comando com um login cada) e um robô rápido (`scripts/research/fx_demo_runner.py`) que roda uma regra pré-registrada em barras ao vivo com o mesmo executor, trava de risco e journal do núcleo do runner. Está rodando a C1 (um controle que o laboratório diz que perde) na conta demo 10139135, só para comparar o trade real com o simulado. A senha do cTID é digitada no prompt do CLI e nunca vai como argumento (como argumento ela aparece em qualquer listagem de processos do computador); o robô nomeia o container e o remove pelo nome, limita o log dele e desiste depois de 3 logins seguidos que falham.
- **Rodada 2, calibração (2026-09-20, o Igor decidiu não parar):** pré-registro em `docs/forex/fast2-preregistration.md` (commitado antes de qualquer resultado, com registro append-only e guarda de hash em `fast2-registry.jsonl`). Todas as famílias são calibradas numa **grade de 212 combinações** (`docs/forex/fast2-grid.json`, gerada por `fx_fast2_grid.py`; as 8 configurações da rodada 1 estão dentro, com parâmetros idênticos, e um teste confere), com a estatística da rodada 1 (bootstrap de dias com máximo-t e placebo com 200 conjuntos) pagando pelo número de combinações. **Três amostras:** descoberta em **2014-11 a 2018-11** (ticks do HistData de outra fonte; 2011-01 a 2014-10 ficou reservado porque o feed cotava spreads fixos de número redondo), réplica em 2019-01 a 2024-08 e confirmação única na amostra congelada 2024-09 a 2026-08. Código: `fx_fast2_lab.py` (`prepare`, `calibrate`, `discover`, `replicate`, `confirm`), `fx_fast2_manifest.py` (qualidade do dado de S0, com checagem de relógio sem oráculo), `fx_fast2_registry.py`. Duas otimizações sem mudar resultado (conferidas contra o relatório da rodada 1): a F3 só calcula as medianas nas barras que passam no teste de corpo (3× mais rápida) e `run_sample` reaproveita barras agregadas entre configurações. Uma réplica placebo das 212 combinações leva ~3 min por núcleo. **S0 resealada antes de qualquer estratégia:** 10 pares e 490 par-meses; o manifesto inclui 477 e exclui 13 (9 GBPUSD e 4 USDCAD) somente por divergência do oráculo, sem buracos, spreads excessivos, relógio inválido ou cotação cruzada. A primeira tentativa de calibração abortou sem produzir resultado porque o calendário de rollover começava em 2015; um teste reproduziu a falha antes de o calendário ser ampliado para 2014. **Calibração concluída em 2026-09-21:** 0,0% de falsa aprovação nos 200 conjuntos placebo (limite 8%), com 42.400 estatísticas finitas; mediana do melhor t = 0,566, p95 = 1,714 e p99 = 2,319. Próximos passos: medir o poder por controle positivo (informativo), descobrir em S0 e, somente se houver aprovadas, replicar em S1 e confirmar uma vez em S2, com registro e relatório commitados em cada etapa. Palpite registrado antes do dado: 2% a 5% de chance de algo passar nas três amostras.
- **Coordenação entre agentes (2026-09-20):** duas sessões, Claude e Codex, mexeram na rodada 2 ao mesmo tempo. O Codex commitou o manifesto de S0, achou dois defeitos reais (a checagem de relógio confundia o domingo parcial na fronteira do mês UTC, e o calendário de rollover começava em 2015, fora de S0) e lançou a calibração 2 minutos antes de mim; a minha, duplicada, foi encerrada e **a do Codex segue** (200 conjuntos, 10 processos). Regra daqui em diante: uma sessão dirige o laboratório por vez; antes de lançar qualquer etapa de `fx_fast2_lab`, conferir `ps` e `backend/data/lab_pre/calibrate.lock` (criado pelas versões novas); nunca deixar arquivo sem commit em `app/` ou `scripts/` (o laboratório recusa rodar com a árvore suja); nunca rodar duas etapas do registro em paralelo. A curva de poder por controle positivo (`fx_fast2_power.py`, informativa) roda depois que a calibração terminar, com as mesmas sementes.
- **Rodada 2, poder do critério (2026-09-21, informativo):** em 40 controles positivos sobre os placebos, F1 chega a 60%–93% de poder com vantagem líquida verdadeira de +0,05 R por trade e a 100% com +0,08 R; F2 chega a 97%–100% entre +0,08 e +0,10 R; F3 exige cerca de +0,15 R para 100%. F5a tem mediana de só 227 trades e, por isso, nunca cruza o mínimo pré-registrado de 300 mesmo com edge injetado: nessa janela ela tende a ser inconclusiva por tamanho amostral. Relatório: `docs/forex/fast2-power-report.md`. O controle não altera critério nem registro e não usa resultado real.
- **Rodada 2, descoberta encerrada em S0 (2026-09-21): 0 das 212 combinações foi aprovada; S1 e S2 não foram tocadas.** Todas as 212 tiveram R médio negativo depois do custo base, todas continuaram negativas com spread em estresse e nenhuma teve ao menos 60% dos pares positivos. A melhor média foi F1a `rr1.5/w0.3-1.2/11:00-16:30`, ainda negativa em −0,0179 R por trade (4.714 trades; −0,0626 R no estresse; 5 de 10 pares positivos). As 48 combinações de F5 ficaram inconclusivas por tamanho amostral/poder, mas também tiveram retorno negativo; as outras 164 foram reprovadas conclusivamente. Pela regra pré-registrada, a rodada para aqui e não abre réplica nem confirmação. Relatório: `docs/forex/fast2-discovery-report.md`.
